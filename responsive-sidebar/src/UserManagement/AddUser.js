...
import { uploadExcelSheet, submitUserDataMultipart, fetchDepartments, fetchRoles, fetchRegions } from './userProfileService'; // Adjust import path as necessary
import './usermgmt.css';
...
const AddUser = () => {
  ...
  const handleChangeExcel = (e) => {
    ...
    setExcelFormData({ ...excelFormData, [name]: files[0] });
  };
  ...
  const handleExcelSubmit = () => {
    if (excelFormData.excelSheet) {
      const formData = new FormData();
      formData.append('excelSheet', excelFormData.excelSheet);

      uploadExcelSheet(formData)
        .then((response) => {
          console.log('Excel sheet uploaded successfully:', response);
          // Handle success UI update or other logic
        })
        .catch((error) => {
          console.error('Error uploading Excel sheet:', error);
          // Handle error UI update or other logic
        });
    } else {
